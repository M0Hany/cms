    ...
    <div class="showroom-products-wrapper">
      ...
      <div class="showroom-products-container">`
        ...
        if (productFunctionCall) {
          htmlCode += `
          <section x-data="products" x-init="console.log('Initializing showroom component', $el.id)" x-intersect.once.margin.300px="${productFunctionCall}">`
        }
        ...
        if (sale) {
          htmlCode += `
                      <span class="old-price" style="padding:5px; color:#616161; text-decoration:line-through;" x-text="product.regular.toFixed(2) + ' ' + (getCookie('showroom_currency') || 'EGP')"></span>`
        }
        ...
        if (sale) {
          htmlCode += `
                            <span class="product-price" style="padding:5px; background-color:#e3262f; color:white;" x-text="product.prix.toFixed(2) + ' ' + (getCookie('showroom_currency') || 'EGP')"></span>`
        } else {
          htmlCode += `
                            <span class="product-price" x-text="product.prix.toFixed(2) + ' ' + (getCookie('showroom_currency') || 'EGP')"></span>`
        }
